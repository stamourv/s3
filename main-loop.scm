...
(define input-file (open-input-file (list path: "~/tmp/test-feed"))) ; TODO fix with tun/tap
...
(define (main-loop) ; TODO lire en xx:xx:xx ? format pour frame ethernet ?
  (let ((input (read input-file)))
	...
	(begin (whole-pkt-set! (list->u8vector input))
	       ...
	       (print pkt))))) ; TODO print it to the tun/tap device, actually, send-frame should do it

(main-loop)


;; TODO get rid of the set! pkt
